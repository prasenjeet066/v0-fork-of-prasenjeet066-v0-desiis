...
export function PostActionsMenu({ post, currentUserId, onPostUpdated, onPostDeleted }: PostActionsMenuProps) {
  ...
  const handleDelete = async () => {
    setIsLoading(true)
    try {
      const { error } = await supabase.from("posts").delete().eq("id", post.id).eq("user_id", currentUserId) // Ensure user can only delete their own posts
  ...
  const handleEdit = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq("id", post.id)
        .eq("user_id", currentUserId) // Ensure user can only edit their own posts
  ...
  const handleBlock = async () => {
    ...
    try {
      ...
      if (!error) {
        ...
        // Refresh the timeline or remove the post from view
        onPostUpdated?.()
      }
    } catch (error) {
